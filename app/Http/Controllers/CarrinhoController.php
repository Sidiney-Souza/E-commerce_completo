...
class CarrinhoController extends Controller
{
    ...
    public function index()
    {
        ...
        //dd([
        //    $pedidos,
        //   $pedidos[0]->itens_pedidos,
        //    $pedidos[0]->_pedidos[0]->produto
        // ]);
    ...
    public function cancelar(Request $request){
        ...
        // UPDATE `itens_pedidos` SET `status`='[CA]' WHERE 1
    ...
    public function remover(){
        ...
        $this->middleware('VerfyCsrfToken');

        $req = Request();
        $idPedido = $req->input('pedido_id');

        //DELETE FROM `pedidos` WHERE $idPedido;
        DB::delete('DELETE FROM pedidos WHERE id = ?', [$idPedido]);
        return redirect()->route('carrinho.index');
